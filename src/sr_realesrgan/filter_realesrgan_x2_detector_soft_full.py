...
TRAIN_STAGE_A = ROOT / "data/crops_sr_realesrgan_full/manifests/quality_filter_realesrgan_x2_full_train_results.json"
VAL_STAGE_A = ROOT / "data/crops_sr_realesrgan_full/manifests/quality_filter_realesrgan_x2_full_val_results.json"
TEST_STAGE_A = ROOT / "data/crops_sr_realesrgan_full/manifests/quality_filter_realesrgan_x2_full_test_results.json"
...
OUT_TRAIN = ROOT / "data/crops_sr_realesrgan_full/manifests/stage_b_soft_realesrgan_x2_full_train_results.json"
OUT_VAL = ROOT / "data/crops_sr_realesrgan_full/manifests/stage_b_soft_realesrgan_x2_full_val_results.json"
OUT_TEST = ROOT / "data/crops_sr_realesrgan_full/manifests/stage_b_soft_realesrgan_x2_full_test_results.json"
OUT_SUMMARY = ROOT / "reports/sr_realesrgan_full/x2/stage_b_soft_full_summary.json"
...
def process_records(records, split_name, hr_by_ann, model):
    ...
    for idx, rec in enumerate(candidates, start=1):
        ...
        accepted_stage_b_soft = (iou_s >= iou_b) and (conf_s >= conf_b - CONF_MARGIN)
        if accepted_stage_b_soft:
            accepted_count += 1
        ...
        results.append({
            ...
            "iou_bicubic": iou_b,
            "iou_sr": iou_s,
            "accepted_stage_b_soft": accepted_stage_b_soft
        })

        if idx % 100 == 0:
            print(f"{split_name}: processed {idx} samples...")

    return results, {"total_stage_a_candidates": len(candidates), "accepted_stage_b_soft": accepted_count}


def main():
    with open(TRAIN_STAGE_A, "r", encoding="utf-8") as f:
        train_stage_a = json.load(f)
    ...
    train_results, train_summary = process_records(train_stage_a, "train", hr_by_ann, model)
    val_results, val_summary = process_records(val_stage_a, "val", hr_by_ann, model)
    test_results, test_summary = process_records(test_stage_a, "test", hr_by_ann, model)
